...
RUN set -eux; \
    ...
    && apt-get update \
    && apt-get install --no-install-recommends -y software-properties-common $gpg_pkgs make bsdmainutils bison flex libelf-dev libssl-dev bc git openssl findutils ca-certificates wget build-essential zlib1g-dev libncurses5-dev libgdbm-dev libnss3-dev libreadline-dev libffi-dev  libdw-dev dwarves\
    && apt-get clean \ 
